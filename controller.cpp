...
void Controller::Start() {
  ...
  SetColor(2);  //设置开始动画颜色
  auto start = new StartInterface();  /// 分配一个StartInterface类start
  start->Action();  //开始动画
  delete start;  //释放内存空间
...
void Controller::Select() {  //选择界面
  ...
  while ((ch = getch())) {
    switch (ch) { //检测输入键
	  ...
	  if (key > 1) { //当此时选中项为第一项时，UP上方向键无效
		  switch (key) {
			case 2: SetCursorPosition(27, 22);//给待选中项设置背景色
			  SetBackColor();
			  std::cout << "简单模式";
			  ...
			  --key;
			  break;
			case 3: SetCursorPosition(27, 24);
			  SetBackColor();
			  std::cout << "普通模式";
	  ...
	  case 80://DOWN下方向键
		if (key < 4) {
		  switch (key) {
			case 1: SetCursorPosition(27, 24);
			  SetBackColor();
			  std::cout << "普通模式";
			  SetCursorPosition(27, 22);
			  SetColor(3);
			...
			case 2: SetCursorPosition(27, 26);
			  ...
			  ++key;
			  break;
			case 3: SetCursorPosition(27, 28);
			  SetBackColor();
			  std::cout << "炼狱模式";
			  SetCursorPosition(27, 26);
			  SetColor(3);
  ...
  switch (key) {  //根据所选选项设置蛇的移动速度，speed值越小，速度越快
	case 1: speed = 135;  // 每 135 移动一次，体现在循环间隔中
	  break;
	case 2: speed = 100;  //
	  break;
	case 3: speed = 60;
...
void Controller::DrawGame() {//绘制游戏界面
  ...
  switch (key) {
	case 1: std::cout << "简单模式";
	  break;
	case 2: std::cout << "普通模式";
	  break;
	case 3: std::cout << "困难模式";
	  break;
	case 4: std::cout << "炼狱模式";
	  break;
	default: break;
  }
...
int Controller::PlayGame() { //游戏二级循环
   ...
   * 返回值2：退出
   */
  auto csnake = new Snake();  /// 创建snake对象
  auto cfood = new Food();  /// 创建food对象
  SetColor(6);
  csnake->InitSnake();
  srand((unsigned)time(nullptr));//  设置随机数种子，如果没有 食物的出现位置将会固定
  ...
  switch (tmp) {
	case 1: return 1;  //重新开始
	case 2: return 2;  //退出游戏
	default: return 2;
  }
}
  ...
  while ((ch = getch())) {
	switch (ch) {
	  case 72://UP
		if (tmp_key > 1) {
		  switch (tmp_key) {
			case 2: SetCursorPosition(34, 21);
			  SetBackColor();
			  std::cout << "继续游戏";
			  SetCursorPosition(34, 23);
			  SetColor(11);
			  ...
			  --tmp_key;
			  break;
			case 3: SetCursorPosition(34, 23);
			  SetBackColor();
			  std::cout << "重新开始";
			  SetCursorPosition(34, 25);
			  SetColor(11);
	  ...
	  case 80://DOWN
		if (tmp_key < 3) {
		  switch (tmp_key) {
			case 1: SetCursorPosition(34, 23);
			  SetBackColor();
			  std::cout << "重新开始";
			  SetCursorPosition(34, 21);
			  SetColor(11);
			  ...
			  ++tmp_key;
			  break;
			case 2: SetCursorPosition(34, 25);
			  SetBackColor();
			  std::cout << "退出游戏";
			  SetCursorPosition(34, 23);
			  SetColor(11);
...
void Controller::Game() { //游戏环境， 一级循环
  Start();  ///  开始界面
  while (true) {    /// 游戏可视为一个无限循环，直到退出游戏时循环结束
	Select();  // 选择界面
	DrawGame();  //绘制游戏界面
